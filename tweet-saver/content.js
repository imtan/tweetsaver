...
(() => {
  ...
  const articleDataMap = new Map(); // tweetId/articleId -> normalized X Article data
  ...
  window.addEventListener("__tweetsaver_article", (e) => {
    storeArticleData(e.detail?.tweetId, e.detail?.data);
  });

  window.addEventListener("__tweetsaver_article_response", (e) => {
    storeArticleData(e.detail?.tweetId, e.detail?.data);
  });
  ...
  function extractTweet(article) {
    ...
    if (article.querySelector("video")) {
      ...
      if (data.id && videoUrlMap.has(data.id)) {
        ...
        data.videoUrls = videoUrlsFromData(vData);
      } else if (data.id) {
        ...
        if (videoUrlMap.has(data.id)) data.videoUrls = videoUrlsFromData(videoUrlMap.get(data.id));
      }
    }

    const articleData = getArticleDataForTweet(data.id);
    if (articleData) {
      data.article = articleData;
      if (!data.card) {
        data.card = {
          url: articleData.url || data.url || location.href,
          domain: "X Article",
          title: articleData.title || "Article",
          description: articleData.previewText || (articleData.text || "").split("\n").find(Boolean) || "",
          imageUrl: articleData.coverImageUrl || (articleData.imageUrls && articleData.imageUrls[0]) || "",
        };
      }
    }
  ...
  function videoUrlsFromData(vData) {
    return [...new Set([
      vData?.mp4Url,
      ...(vData?.allVariants || []).map((v) => v.url),
    ].filter(Boolean))];
  }
  ...
  function storeArticleData(key, data) {
    if (!data) return;
    for (const k of [key, data.tweetId, data.restId].filter(Boolean)) articleDataMap.set(String(k), data);
  }

  function articlePageId() {
    const m = location.pathname.match(/\/i\/article\/(\d+)|\/[^/]+\/article\/(\d+)/);
    return m && (m[1] || m[2]);
  }

  function getArticleDataForTweet(tweetId) {
    const pageId = articlePageId();
    const keys = [tweetId, pageId].filter(Boolean).map(String);
    for (const key of keys) {
      if (articleDataMap.has(key)) return articleDataMap.get(key);
      window.dispatchEvent(new CustomEvent("__tweetsaver_query_article", { detail: { tweetId: key } }));
      if (articleDataMap.has(key)) return articleDataMap.get(key);
    }
    // Fallbacks (size-1 guess / DOM scrape) only on a dedicated article page,
    // never for timeline tweets — otherwise an unrelated cached article leaks in.
    if (!pageId) return null;
    if (articleDataMap.size === 1) return articleDataMap.values().next().value;
    return extractArticleFromDom();
  }

  function extractArticleFromDom() {
    if (!articlePageId()) return null;
    const main = document.querySelector('main[role="main"]') || document.body;
    const title = (main.querySelector("h1")?.innerText || "").trim();
    const best = [...main.querySelectorAll("article, section, div")]
      .filter((el) => el.offsetParent !== null)
      .map((el) => ({ el, text: cleanArticleText(el.innerText || "") }))
      .filter((x) => x.text.length > 500)
      .sort((a, b) => b.text.length - a.text.length)[0];
    if (!best) return null;
    const imageUrls = collectArticleImages(best.el);
    return {
      restId: articlePageId(),
      title: title || best.text.split("\n").find(Boolean) || "X Article",
      previewText: "",
      text: best.text,
      url: location.href,
  ...
  function cleanArticleText(text) {
    const junk = new Set(["Post", "Reply", "Repost", "Like", "Share", "Bookmark", "Translate post", "Show more"]);
    return text.split("\n").map((s) => s.trim()).filter((s) => s && !junk.has(s)).join("\n");
  }
  ...
  function collectArticleImages(root) {
    ...
    for (const img of root.querySelectorAll("img")) {
      if (!img.src || !img.src.includes("pbs.twimg.com")) continue;
      if (/profile_images|emoji/i.test(img.src)) continue;
      const u = upgradeImageUrl(img.src);
      if (!out.includes(u)) out.push(u);
    }
    return out;
  ...
  async function saveTweet(article, btn) {
    ...
    return saveExtractedTweet(tweet, btn);
  }

  async function saveArticlePage(btn) {
    const article = getArticleDataForTweet(null);
    const id = articlePageId() || article?.tweetId || article?.restId;
    if (!article || !id) { showToast("Could not identify article", true); return; }
    const match = location.pathname.match(/^\/([^/]+)\/article\//);
    const username = article.username || (match && match[1]) || "article";
    return saveExtractedTweet({
      id: String(id),
      username: username === "i" ? "article" : username,
      displayName: article.authorName || username,
      text: article.previewText || article.title || "",
      timestamp: article.publishedAt || "",
      isoTime: article.publishedAt || "",
      ...
      card: {
        ...
        description: article.previewText || (article.text || "").split("\n").find(Boolean) || "",
        imageUrl: article.coverImageUrl || (article.imageUrls && article.imageUrls[0]) || "",
      },
      article,
    }, btn);
  }

  function articleMarkdown(article, tweet) {
    if (!article) return "";
    const lines = [
      `# ${article.title || "X Article"}`,
      "",
      article.previewText || "",
      "",
      article.text || "",
      "",
      `Source: ${article.url || tweet.url}`,
    ];
    if (article.imageUrls && article.imageUrls.length) lines.push("", "## Images", ...article.imageUrls);
    return lines.filter((line, i, arr) => line || arr[i - 1]).join("\n");
  }

  async function renderArticleImage(article, tweet) {
    const dark = isDarkMode();
    const theme = dark
      ? { bg:"#15202b", card:"#192734", text:"#e7e9ea", sub:"#8b98a5", border:"#38444d", accent:"#1d9bf0" }
      : { bg:"#f0f2f5", card:"#ffffff", text:"#0f1419", sub:"#536471", border:"#e1e8ed", accent:"#1d9bf0" };
    const W = 760, PAD = 40, BODY = 18, LINE = 29;
    const FONT = '"Segoe UI", "Helvetica Neue", Arial, sans-serif';
    const tmp = document.createElement("canvas");
    const tctx = tmp.getContext("2d");
    const innerW = W - PAD * 2;

    tctx.font = `bold 30px ${FONT}`;
    const titleLines = wrapText(tctx, article.title || "X Article", innerW);
    tctx.font = `16px ${FONT}`;
    const previewLines = article.previewText ? wrapText(tctx, article.previewText, innerW) : [];
    tctx.font = `${BODY}px ${FONT}`;
    const bodyLines = wrapText(tctx, article.text || "", innerW);
    const sourceLines = wrapText(tctx, article.url || tweet.url || location.href, innerW);
    const H = PAD + titleLines.length * 39 + (previewLines.length ? 18 + previewLines.length * 24 : 0) + 26 + bodyLines.length * LINE + 34 + sourceLines.length * 20 + PAD;

    const canvas = document.createElement("canvas");
    canvas.width = W;
    canvas.height = Math.min(H, 32760); // ponytail: browser canvas limit; split pages if articles ever exceed this.
    const ctx = canvas.getContext("2d");
    ctx.fillStyle = theme.bg;
    ctx.fillRect(0, 0, W, canvas.height);
    ctx.fillStyle = theme.card;
    roundRect(ctx, 20, 20, W - 40, canvas.height - 40, 18);
    ctx.fill();
    ...
    for (const line of titleLines) { ctx.fillText(line, PAD, y + 30); y += 39; }
    if (previewLines.length) {
      y += 18;
      ctx.fillStyle = theme.sub;
      ctx.font = `16px ${FONT}`;
      for (const line of previewLines) { ctx.fillText(line, PAD, y + 16); y += 24; }
    }
    y += 26;
    ctx.fillStyle = theme.text;
    ctx.font = `${BODY}px ${FONT}`;
    for (const line of bodyLines) {
      if (y + LINE > canvas.height - PAD - 40) break;
      ctx.fillText(line, PAD, y + BODY);
      y += LINE;
    }
    y += 20;
    ...
    for (const line of sourceLines) {
      if (y + 20 > canvas.height - PAD) break;
  ...
  async function saveExtractedTweet(tweet, btn) {
      ...
      const resp = await api.runtime.sendMessage({
        ...
        data: {
          tweet: {
            ...
            quote: tweet.quote, card: tweet.card, article: tweet.article,
            savedAt: new Date().toISOString(),
          },
          // For Downloads
          archivePngDataUrl: archiveDataUrl,
          articlePngDataUrl: articleDataUrl,
  ...
  function injectArticlePageButton() {
    if (!articlePageId() || document.querySelector("#tweet-saver-article-page-btn")) return;
    const btn = document.createElement("button");
    btn.id = "tweet-saver-article-page-btn";
    btn.className = "tweet-saver-btn tweet-saver-page-btn";
    btn.innerHTML = SAVE_ICON;
    btn.setAttribute("data-tooltip", `Save article (${shortcutLabel()})`);
    btn.addEventListener("click", (e) => {
      e.preventDefault(); e.stopPropagation();
      saveArticlePage(btn);
    });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (keyMatch && modMatch) {
      ...
      } else if (articlePageId()) {
        const btn = document.querySelector("#tweet-saver-article-page-btn");
        if (btn) saveArticlePage(btn);
      }
    }
  });
